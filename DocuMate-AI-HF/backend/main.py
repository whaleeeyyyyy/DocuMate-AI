...
from services.hf_agent_service import HuggingFaceAgentService
...
app = FastAPI(
    ...
    description="FastAPI backend for DocuMate AI, integrating Hugging Face and Supabase.",
    version="0.1.0",
)
...
hf_agent_service = HuggingFaceAgentService(supabase_service, hf_service)
...
async def upload_pdf(
    ...
    try:
        ...
        # 3. Generate Summary and Entities using Hugging Face concurrently
        # This speeds up the response by running AI tasks in parallel
        summary_task = hf_service.generate_summary(cleaned_text)
        entities_task = hf_service.extract_entities(cleaned_text)
        ...
        for ent in raw_entities:
            ...
            if text_to_find and label:
                ...
                # For more robust NER, consider a dedicated library or more advanced prompting.
        ...
        for i, chunk_text in enumerate(chunks):
            embedding = await hf_service.get_embedding(chunk_text)
...
async def ask_question(
    ...
    try:
        ...
        query_embedding = await hf_service.get_embedding(request.question)
        ...
        # Combine relevant chunks into a single context string for Hugging Face QA
        context = "\n\n".join(relevant_chunks)

        # 3. Generate answer using Hugging Face, grounded in the retrieved context
        answer = await hf_service.generate_answer(request.question, context)
...
async def semantic_search(
    ...
    try:
        ...
        query_embedding = await hf_service.get_embedding(request.query)
...
@app.post("/agent-query", response_model=AgentQueryResponse, summary="Ask a Complex Query using Hugging Face Agent with Tools")
async def agent_query(
    request: AgentQueryRequest,
    user_id: str = Depends(get_current_user_id)
):
    """
    Processes a complex user query using a Hugging Face agent that can decide to use
    various tools (e.g., get summary, get entities, semantic search) to formulate an answer.
    """
    try:
        # The agent service handles the interaction loop with tools
        agent_response = await hf_agent_service.invoke_agent(request.document_id, request.query)
...
@app.get("/check-hf", summary="Check Hugging Face API Status")
@app.get("/check-gemini", summary="Check Hugging Face API Status (compat)")
async def check_hf_status():
    """
    Checks if the Hugging Face Inference API is accessible and functional.
    """
    try:
        ok = await hf_service.health_check()
        if ok:
            return {"status": "ok", "message": "Hugging Face API is working correctly."}
        raise Exception("Hugging Face API returned an unexpected response.")
    except Exception as e:
        print(f"Hugging Face API check failed: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Hugging Face API check failed: {e}. Please check your API token and network connection."
        )
